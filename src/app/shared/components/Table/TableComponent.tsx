...
interface propsTable {
  ...
  onExportExcel?: () => void;
  transformExcel?: boolean; // Si es True trnasformara las key que se pasen,
  transformRegex?: { key: string; regex: string }[];
    ...
    <>
      <TableRow role="checkbox" sx={funcionDeRenderizado(row)}>
        ...
        {props.columns?.map((element, index) => (
          <TableCell align="left" key={index} sx={{ "@media (max-width: 640px)": { padding: "0rem 2rem" } }}>
...
function EnhancedTableHead(props: any) {
  const { order, orderBy, onRequestSort, setFilterSelected ,headerBackgroundColor } = props;
...
export const TableComponent = (props: propsTable) => {
  ...
  const excelExport = () => {
    if (_exporter.current) {
      if (props.transformExcel) {
        let data = _exporter.current.props.data;
        data = data.map((d) => {
          props.transformRegex.forEach(({ key, regex }) => {
            if (Object.hasOwnProperty.call(d, key)) {
              d[key] = d[key].replace(/[\d-]/g, "");
            }
          });
          return d;
        });
        console.log(data);
        _exporter.current.save(data);
      } else {
  ...
  return (
    <div>
          ...
          <div className="flex rounded-lg items-center px-2 w-full my-2 bg-secondaryNew shadow-elevation-4">
            ...
            <div className=" text-gray-900 dark:text-gray-200 px-2 w-full pb-4">
              <div className="flex items-end">
                <Search className="fill-textColor" />
                <TextField
                  id="busqueda"
                  fullWidth
                  value={searchString}
      ...
      <TableContainer
        sx={{ backgroundImage: "linear-gradient(180deg, var(--table-color-top), var(--table-color-bottom))" }}
        component={Paper}
          ...
          <TableBody>
            {props.dataInfo?.length > 0
              ? (rowsPerPage > 0
                ...
                return <RowBody {...props} row={row} key={_.get(row, props.IDcolumn)} />;
              })
              : DataOpen.map((row: any) => <RowBody {...props} row={row} key={_.get(row, props.IDcolumn)} />)}
          </TableBody>
          <TableFooter>
            <TableRow>
              <TablePagination
