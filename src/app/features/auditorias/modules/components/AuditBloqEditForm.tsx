...
export const AuditBloqEditForm = ({ callbackFunction, setOpenPopup, auditBloqSelect }: props) => {
  ...
  useEffect(() => {
    setValue("name", auditBloqSelect.bloq.name);
    for (let index = 0; index < auditBloqSelect.bloq.itemBloq.length; index++) {
      const element = auditBloqSelect.bloq.itemBloq[index];
      addValues(false);
      setValue(`itemBloq.${index}.item.name`, element.item.name);
    }
    console.log(auditBloqSelect.bloq.itemBloq);
  }, []);
