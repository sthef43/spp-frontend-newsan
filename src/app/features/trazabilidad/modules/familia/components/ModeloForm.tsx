...
export const ModeloForm = ({ setOpenPopup, editState, refresh, familiaId }: props) => {
  ...
  const addProducto = async (e) => {
    ...
    if (result) {
      openNotificationUI("Se agrego el modelo correctamente", "success");
      setOpenPopup(false);
      refresh();
    }
  };
        ...
        <div className="flex flex-col" style={{ height: "100%" }}>
          <div className="py-2 grid grid-cols-1 gap-10 overflow-auto" style={{ flex: "1 1 90%" }}>
            ...
            <Controller
              ...
              render={({ field, fieldState: { error } }) => (
                <FormControl fullWidth variant="outlined" error={!!error}>
                  <InputLabel>SemiElaborado</InputLabel>
                  <Input {...field} />
                  {!!error && <FormHelperText>{error.type}</FormHelperText>}
                </FormControl>
              )}
