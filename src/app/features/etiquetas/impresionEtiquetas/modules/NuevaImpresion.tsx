...
import { CodigoZplByModelo } from "../components/CodigoZplByModelo";

export const NuevaImpresion = () => {
  ...
  useEffect(() => {
    ...
    const ultimoNumImprimir = ultimoNumeroImpresoDefinitivo + parseInt(cantidadImprimirAux);
    if (watchCantidadImprimir == "0" && ultimoNumImprimir == 99999) {
      openNotificationUI("Limite de impresiones. Cambie prefijo.", "error");
    }
    if (ultimoNumImprimir > 99999) {
      openNotificationUI("El numero no puede superar los 99999", "warning");
      setValue("cantidadImprimir", "0");
      //setValue("ultimoNumeroImprimir", 0);
    } else setValue("ultimoNumeroImprimir", ultimoNumImprimir);
  ...
  const completoCampos = () => {
    if (getValues("impresora") && ascendente != null && getValues("cantidadImprimir")) return true;
    else {
      openNotificationUI("Elija Numeración, Impresora y Cantidad a Imprimir.", "warning");
      return false;
    }
  };
  ...
  const armarCodigoZpl = async () => {
    const desdeNumero: number = ultimoNumeroImpresoDefinitivo + 1;
    const hastaNumero: number = desdeNumero + parseInt(getValues("cantidadImprimir")) - 1;
    ...
    console.log("desdeNumero " + desdeNumero);
  ...
  const armarObjetoImpresion = () => {
    ...
    const impresion = {
      ...
      numeradorDesde: ultimoNumeroImpresoDefinitivo + 1,
      numeradorHasta: ultimoNumeroImpresoDefinitivo + 1 + parseInt(getValues("cantidadImprimir")) - 1,
      impresoFecha: moment().format(),
      idUsuario: user.id,
      idProducto: productoSeleccionado.id,
      idFamilia: familiaSeleccionada.id,
      tipoEtiqueta: etiquetaSeleccionada.tipoEtiqueta,
      prefijo: etiquetaSeleccionada.prefijo
    };
    return impresion;
  };
  ...
  const verificarTipoEtiquetaYArmarCodigoZpl = async () => {
    let codiZpl = "";
    const ListTipoEtiqueta = unwrapResult(await dispatch(ZPL_TipoEtiquetasSliceRequests.getAllRequest()));
    const tipoEtiqueta = ListTipoEtiqueta.find((x) => x.id == etiquetaSeleccionada.tipoEtiqueta);

    if (tipoEtiqueta.descripcionTipoEtiqueta == "Modelo Base") {
      const codigoZplByModelo = new CodigoZplByModelo();
      //codiZpl = codigoZplByModelo.getCodigoZpl(etiquetaSeleccionada);
      //Armo la logica para armar la etiqueta de modelo
    } else if (tipoEtiqueta.descripcionTipoEtiqueta == "Energia") {
      //armo la logica para armar la etiqueta de energia
    } else if (tipoEtiqueta.descripcionTipoEtiqueta == "Caja") {
      //Armo la logica para arma la etiqueta de caja
    } else {
      codiZpl = await armarCodigoZpl();
    }
  ...
  const print = async () => {
    if (watchCantidadImprimir == "0") {
      openNotificationUI("Elija cantidad a imprimir", "warning");
      return;
    }
    if (completoCampos()) {
      const codiZpl = await verificarTipoEtiquetaYArmarCodigoZpl();

      if (codiZpl == "No acepta") {
        openNotificationUI("Rechazado con exito", "info");
        return;
      }
