...
export class WhatsappMsgAppUserService extends GenericService<IWhatsappMsgAppUser> {
  ...
  public async GetAllByOpcionAsign(opcionId: number, whatsappMsgId: number): Promise<IWhatsappMsgAppUser[]> {
    return new Promise<IWhatsappMsgAppUser[]>((resolve, reject) => {
      axios
        .get<IWhatsappMsgAppUser[]>(
          `${import.meta.env.VITE_API_URL}/${this.Url}/GetAllByOpcionAsign/${opcionId}/${whatsappMsgId}`
        )
        .then((response) => {
  ...
  public async GetAllUsersWithNoAssigment(
    opcionId: number,
      ...
      axios
        .get<IAppUser[]>(
          `${import.meta.env.VITE_API_URL}/${
            this.Url
          }/GetAllUsersWithNoAssigment/${opcionId}/${whatsappMsgId}/${plantId}`
        )
        .then((response) => {
          resolve(response.data);
        })
